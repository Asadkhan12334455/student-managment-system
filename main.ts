...
const ranNum: number = Math.floor(1000 + Math.random() * 90000);
...
if (CourseFees === paymentAmount) {
  console.log(
    chalk.green.bold.italic(
      `Congratilation you have Successfully enrolled in ${answer.Courses}`
    )
  );
  ...
  if (ans.Select === "View Status") {
    ...
    console.log(
      chalk.magenta.bold.underline(`Student Name: ${answer.Student}`)
    );
    console.log(chalk.magenta.bold.underline(`Student ID: ${ranNum}`));
    console.log(chalk.magenta.bold.underline(`Course: ${answer.Courses}`));
    console.log(
      chalk.magenta.bold.underline(`Course Fee Paid: ${paymentAmount}`)
    );
    console.log(
      chalk.magenta.bold.underline(`Balance ${(myBlance += paymentAmount)}`)
    );
  } else {
